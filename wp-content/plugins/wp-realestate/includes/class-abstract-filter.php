		...
		if ( ! is_wp_error( $terms ) && ! empty ( $terms ) ) {
			...
			foreach ($terms as $term) {
			    ...
			    $output .= '<li class="list-item">';
			        ...
			        $child_output = self::hierarchical_tax_tree($term->slug, $next_depth, $input_name, $key, $field, $selected, $input_type);
			        if ( $child_output ) {
		...
		if ( ! is_wp_error( $terms ) && ! empty ( $terms ) ) {
			foreach ($terms as $term) {
			    ...
			    $output .= '<option value="'.esc_attr($term->slug).'" '.$selected_html.'>';
			  	...
			  	$output .= self::hierarchical_tax_option_tree($term->slug, $next_depth, $input_name, $key, $field, $selected);
			}
		}
