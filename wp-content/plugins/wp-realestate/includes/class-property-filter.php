...
class WP_RealEstate_Property_Filter extends WP_RealEstate_Abstract_Filter {
	...
	public static function get_meta_filter($params) {
		...
		// Home area from
		if ( ! empty( $params['filter-home_area-from'] ) ) {
			$meta_query[] = array(
				'key'       => WP_REALESTATE_PROPERTY_PREFIX . 'home_area',
				'value'     => sanitize_text_field( wp_unslash($params['filter-home_area-from']) ),
				'compare'   => '>=',
				'type'      => 'NUMERIC',
			);
		}
	    ...
	    // Home area to
	    if ( ! empty( $params['filter-home_area-to'] ) ) {
		    $meta_query[] = array(
			    'key'       => WP_REALESTATE_PROPERTY_PREFIX . 'home_area',
			    'value'     => sanitize_text_field( wp_unslash($params['filter-home_area-to']) ),
			    'compare'   => '<=',
			    'type'      => 'NUMERIC',
		    );
	    }
	    ...
	    // Lot area from
	    if ( ! empty( $params['filter-lot_area-from'] ) ) {
		    $meta_query[] = array(
			    'key'       => WP_REALESTATE_PROPERTY_PREFIX . 'lot_area',
			    'value'     => sanitize_text_field( wp_unslash($params['filter-lot_area-from']) ),
			    'compare'   => '>=',
			    'type'      => 'NUMERIC',
		    );
	    }
	    ...
	    // Lot area to
	    if ( ! empty( $params['filter-lot_area-to'] ) ) {
		    $meta_query[] = array(
			    'key'       => WP_REALESTATE_PROPERTY_PREFIX . 'lot_area',
			    'value'     => sanitize_text_field( wp_unslash($params['filter-lot_area-to']) ),
			    'compare'   => '<=',
			    'type'      => 'NUMERIC',
		    );
	    }
