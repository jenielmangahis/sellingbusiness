	...
	public static function custom_columns_manage( $column ) {
		...
		switch ( $column ) {
			...
			case 'property_status':
				...
				$statuses = WP_RealEstate_Property::property_statuses();

				$status_text = $status;
				if ( !empty($statuses[$status]) ) {
					$status_text = $statuses[$status];
				}
				echo sprintf( '<a href="?post_type=property&post_status=%s">%s</a>', esc_attr( $post->post_status ), '<span class="status-' . esc_attr( $post->post_status ) . '">' . esc_html( $status_text ) . '</span>' );
				break;
		}
	}
