...
function homeo_agency_display_nb_properties($post) {
    $agents = WP_RealEstate_Query::get_agency_agents($post->ID, array(
        'post_per_page' => -1,
        'fields' => 'ids'
    ));
    $agent_count = !empty($agents->found_posts) ? $agents->found_posts : 0;
    $count_properties = 0;
    if ( !empty($agents->posts) ) {
        $properties = WP_RealEstate_Query::get_agents_properties(array(
            'agent_ids' => $agents->posts,
            'post_per_page' => -1,
            'fields' => 'ids'
        ));
        $count_properties = !empty($properties->found_posts) ? $properties->found_posts : 0;
    }
